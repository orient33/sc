...
public class PowerComputeService extends IntentService {
	...
	private static int sPreBatteryStatus = -1;
	...
	public void onCreate() {
		...
		log_e("sAcChargingCount:" + sAcChargingCount + " sUsbChargingCount:"
				+ sUsbChargingCount);
	}
	...
	protected void onHandleIntent(Intent intent) {
		...
		// 判断手机电池状态是否变化 //去除放电状态
		if (batteryStatus == sPreBatteryStatus
				&& batteryStatus == BatteryManager.BATTERY_STATUS_CHARGING) {
			log_e("battery status :" + batteryStatus+" plugged:"+batteryPlugged);
			...
			// 把重置手机状态电池状态
			sPreBatteryStatus = batteryStatus;
			revertBatteryChargeInfo(sBatteryChargeInfo, -1, batteryLevel);
		}
	}
	
	private void revertBatteryChargeInfo(BatteryChargeCell batteryChargeInfo,int chargeStatus,int batteryLevel){
		batteryChargeInfo.chargeStatus = chargeStatus;
