...
public class OptimizerActivity extends Activity implements View.OnClickListener{
	...
	public static class OptFragment extends MyFragment implements View.OnClickListener{
...
//		ListView mmListView;
		...
		private void refreshCacheInfo(){
			for(OneCheckItem is : list){
				...
				is.useResult(rs);
//				mmHandler.obtainMessage(MSG_UPDATE_UI, is.nameId, rs);
			}
		}
		...
		class MyAdapter extends BaseAdapter{
			void bind(){
				if (list.size() > 0)
					return;
				Fragment ff[] = { ClearCacheFragment.get(), ClearCacheFragment.get(), KillProcessFragment.get() };
