...
public class PowerManagerActivity extends Activity implements Handler.Callback {
	...
	private BatteryWaterWaveView mBatteryLogo;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		sAcChargeSpeed = computeChargeSpeedByDb(this,
				BatteryManager.BATTERY_PLUGGED_AC);
		sUsbChargeSpeed = computeChargeSpeedByDb(this,
	...
	private void registerBatteryStatusReceiver(Context context) {
		...
		context.registerReceiver(mBatteryStatusReceiver, new IntentFilter(
				BatteryInfoHelper.BATTERY_CHANGE_MSG));
	}
	...
	private void setUpViewsData() {
		mBatteryLogo = (BatteryWaterWaveView) this
				.findViewById(R.id.battery_logo);
		mBatteryLogo.animateWave();
	...
	private ArrayList<BatteryModeCell> loadLocalData(Context context) {
		...
		boolean isFirstLaucher = SystemPreference.getInstance(context)
				.isPowerManagerFirstLaunch();
		if (!isFirstLaucher) {
		...
		try {
			if (!TextUtils.isEmpty(jsonData)) {
				...
				JSONArray powerPlanJa = new JSONArray(
						powerPlanJo
								.optString(BatteryModeInfoKey.DEFAULT_BATTERY_MODE));
				listData = new ArrayList<BatteryModeCell>();
				for (int i = 0; i < powerPlanJa.length(); i++) {
					BatteryModeCell batteryModeInfo = DataEntityFactory
							.createBatteryModeEntity(powerPlanJa.optString(i));
	...
	private String readBatteryModeDataByAssets() {
		try {
			BufferedReader br = new BufferedReader(new InputStreamReader(this
					...
					.open("power/power_default_battery_mode.json"), "utf-8"));
	...
	private int computeChargeSpeedByDb(Context context, int chargeStatus) {
		int chargeSpeed = 0;
		ArrayList<BatteryChargeCell> batteryChargeList = BatteryChargeCell
		...
		if (batteryChargeList != null && batteryChargeList.size() > 0) {
			...
			log_e("computeChargeSpeedByDb batteryChargeList size:"
					+ batteryChargeList.size() + "	chargeStatus:"
					+ chargeStatus + " chargeSpeed:" + chargeSpeed);
		}
		return chargeSpeed;
	}

	private void updateBatteryStatusUi(int batteryStatus, int batteryLevel,
			int hour, int minute) {
		boolean isCharging = (batteryStatus == BatteryManager.BATTERY_STATUS_CHARGING) ? true
				: false;
		mBatteryLogo.setBatteryCharging(isCharging);
		mBatteryLogo.setBatteryLevel(batteryLevel);
		mBatteryLogo.setBatteryHour(hour);
		mBatteryLogo.setBatteryMinute(minute);
	}
	...
	public boolean handleMessage(Message msg) {
		if (msg.what == 1) {
			...
			if (batteryStatus == BatteryManager.BATTERY_STATUS_CHARGING) {
				if (batteryInfo.batteryPlugged == BatteryManager.BATTERY_PLUGGED_AC) {
					if (sAcChargeSpeed > 0) {
						batteryTime = (batteryInfo.batteryScale - batteryInfo.batteryLevel)
								* sAcChargeSpeed;
					} else {
						batteryTime = BatteryInfoHelper.getInstance()
								.getDefaultAcChargeSurplusTime();
					}
				} else if (batteryInfo.batteryPlugged == BatteryManager.BATTERY_PLUGGED_USB) {
...
//				} else {
					batteryTime = BatteryInfoHelper.getInstance()
							.getDefaultDisChargeSurplusTime();
//				}
			}
	...
	private void onActionbarClick(View v) {
		synchronized (PowerManagerActivity.class) {
			...
			if (isCreateBatteryMode) {
				...
				BatteryModeCell batteryModeInfo = BatteryModeCell
						.createNewBatteryMode(mContext);
	...
	protected void onDestroy() {
		super.onDestroy();
		mBatteryLogo.stopAnimateWaving();
